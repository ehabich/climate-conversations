...
def getEnt(
    ent_list,
):  # [[[start_id, end_id, ent0][start_id, end_id, ent1][...]],[sen2...]...]
    e_list = []
    for sec in ent_list:
        for sen in sec:
...
def PrintInformation(keys, allcnt):
    ...
    cnt = 0
    first = 0.0
    for _key, val in keys:
        if val >= 10:
            cnt += 1
            first += val
    print(
        "appearance > 10 cnt: %d, percent: %f" % (cnt, first / allcnt)
    )  # 416,303

    # first 30,000, last freq 31
    if len(keys) > 30000:
        first = 0.0
        for _k, v in keys[:30000]:
            first += v
        print(
            "First 30,000 percent: %f, last freq %d"
            % (first / allcnt, keys[30000][1])
        )

    # first 50,000, last freq 383
    if len(keys) > 50000:
        first = 0.0
        for _k, v in keys[:50000]:
            first += v
        print(
            "First 50,000 percent: %f, last freq %d"
            % (first / allcnt, keys[50000][1])
        )

    # first 100,000, last freq 107
    if len(keys) > 100000:
        first = 0.0
        for _k, v in keys[:100000]:
            first += v
        print(
            "First 100,000 percent: %f, last freq %d"
            % (first / allcnt, keys[100000][1])
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default="data/cord-19/train.label.jsonl",
        help="File to deal with",
    )  # key: "text":2 d list, "summary":1d list; "label":1d list; "entity":3d list :[[[start_id, end_id, ent0][start_id, end_id, ent1][...]],[sen2...]...]
    parser.add_argument(
        "--dataset", type=str, default="cord-19", help="dataset name"
    )
