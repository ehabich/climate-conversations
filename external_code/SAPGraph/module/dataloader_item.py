...
class DatasetItem(object):
    ...
    def __init__(
        ...
        # Process the article
        if isinstance(article_sents, list) and isinstance(
            article_sents[0], list
        ):
        ...
        for sent in self.original_article_sents:
            ...
            self.enc_sent_len.append(
                len(article_words)
            )  # store the length before padding
            self.enc_sent_input.append(
                [vocab.word2id(w.lower()) for w in article_words]
        ...
        for n in self.sec_name_list:
            split_n = n.split()
            self.enc_sec_name.append(
                [vocab.word2id(w.lower()) for w in split_n]
            )
        ...
        self.label = label
        label_shape = (
            len(self.original_article_sents),
            len(label),
        )  # [N, len(label)]
        self.label_matrix = np.zeros(label_shape, dtype=int)
        if label != []:
            self.label_matrix[
                np.array(label), np.arange(len(label))
    ...
    def _get_section_level_nodes(self, name_list):
            ...
            for kw_i in range(len(keywords)):
                for w in keywords[kw_i]:
                    if n_str.find(w) != -1:
                        res_list[
                            kw_i
                        ] += 1  # vote: if found keyword_i, then add 1 to keyword_i's index
            if sum(res_list) == 0:
                return 4
            else:
            ...
            if n_tmp:
                if len(n_tmp) == 1:  # it's a 1level section
                    ...
                    if n_tmp[0] not in sec_i.keys():
                        sec_i[n_tmp[0]] = {
                            "sec_node_id": normal_id,
                            "parent_node": sec_i[n_tmp[-1]]["sec_node_id"],
                            "sec_cls": cls,
                        }
                        normal_id += 1
                    ...
                    len(n_tmp) >= 3
                ):  # it's a >=3level section, >=3level must not be a super node
                    cls = sec_cls(" ".join(n_tmp))
                    if n_tmp[-1] not in sec_i.keys():
                        sec_i[n_tmp[-1]] = {
                            "sec_node_id": super_id,
                    ...
                    if n_tmp[-2] not in sec_i.keys():
                        ...
                        super_id -= 1
                    if (
                        n_tmp[-3] not in sec_i.keys()
                    ):  # change here if need all levels
                        sec_i[" ".join(n_tmp[:-2][::-1])] = {
                            "sec_node_id": normal_id,
                            "parent_node": sec_i[n_tmp[-2]]["sec_node_id"],
                            "sec_cls": cls,
                        }
                        normal_id += 1
            else:
        ...
        for k, v in sec_i.items():
            ...
            self.sec_level_list.append(
                [v["sec_node_id"], v["parent_node"], v["sec_cls"]]
            )
