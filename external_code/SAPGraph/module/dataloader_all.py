...
from external_code.SAPGraph.module.dataloader_item import *
...
class DatasetAll(torch.utils.data.IterableDataset):
    ...
    def __init__(
        ...
        self.filterids.append(
            vocab.word2id("[PAD]")
        )  # keep "[UNK]" but remove "[PAD]"
        lowtfidf_num = 0
    ...
    def pad_label_m(self, label_matrix):
        label_m = label_matrix[
            : self.doc_max_timesteps, : self.doc_max_timesteps
        ]
        N, m = label_m.shape
    ...
    def AddEntNode(self, G, ent_id):
        ...
        for sen in ent_id:
            for item in sen:
                out = []
                for wid in item:
                    if wid not in self.filterids or wid == self.vocab.word2id(
                        "[PAD]"
                    ):
                        out.append(1)
                    else:
    ...
    def CreateGraph(
        ...
        G.ndata["dtype"][tsen_nodes:] = (
            torch.ones(sec_num) * 2
        )  # tensor[2,2,2...]
        ...
        map_nid_sensec = [
            secid2nid[sent2sec[i]] - sentid2nid[i] for i in range(len(sent2sec))
        ]
        ...
        for i in range(len(sent2sec)):
            ...
            for enti in tid2nid:
                ...
                for wid in enti:
                    if (
                        wid in sent_pad[i]
                        and self.vocab.id2word(wid) in sent_tfw.keys()
                    ):
                ...
                if add_e:
                    ...
                    G.add_edges(
                        ...
                        sent_nid,
                        data={
                            "tffrac": torch.LongTensor([tfidf_box]),
            ...
            if secid < len(norsec_ids):
                ...
                e_w = (
                    1
                    - 2
                    * min(sent_relative_pos, sent_num - sent_relative_pos)
                    / sent_num
                )
                ...
                G.add_edges(
                    ...
                    secnid,
                    data={
                        "tffrac": torch.LongTensor([e_w_box]),
        ...
        for s in range(len(docLen)):
                ...
                for j in range(i, len(sim_mat)):
                    ...
                    if sim_mat[i][j] > thres_sim:
                        ...
                        G.add_edges(
                            ...
                            sentid2nid[j + offset],
                            data={
                                "tffrac": torch.LongTensor([sim_box]),
            ...
            offset = sum(docLen[:s])
        # add sent data
        G.nodes[sentid2nid].data["words"] = torch.LongTensor(
            sent_pad
        )  # [N, seq_len]
        G.nodes[sentid2nid].data["position"] = (
            torch.arange(1, N + 1).view(-1, 1).long()
        )  # [N, 1]
        G.nodes[sentid2nid].data["label"] = torch.LongTensor(
            label
        )  # [N, doc_max]
        G.nodes[sentid2nid].data["sec_nid_offset"] = torch.LongTensor(
            map_nid_sensec
        )
        ...
        for i in range(len(sec_level)):
            ...
            if parent_id == 0:
                ...
                try:
                    parent_ind = [
                        j
                        for j in range(len(sec_level))
                        if sec_level[j][0] == parent_id
                    ][0]
                    """
                    # add all edge weight as 1
                    ...
                    sec_e_w = 1 - 2 * min(
                        sec_relative_pos,
                        len(level_dic[parent_id]) - sec_relative_pos,
                    ) / len(level_dic[parent_id])
                    sec_e_w_box = np.round(sec_e_w * 9)
                    G.add_edges(
                        secid2nid[i],
                        secid2nid[parent_ind],
                        data={
                            "tffrac": torch.LongTensor([sec_e_w_box]),
                            "dtype": torch.Tensor([3]),
                        },
                    )
                    G.add_edges(
                        secid2nid[parent_ind],
        ...
        if len(topsec_ids):
            ...
            for i in range(len(topsec_ids) - 1):
                if topsec_ids[i] >= len(secid2nid) or topsec_ids[i + 1] >= len(
                    secid2nid
                ):
                    continue
                G.add_edges(
                    secid2nid[topsec_ids[i]],
                    secid2nid[topsec_ids[i + 1]],
                    data={
                        "tffrac": torch.LongTensor([1]),
                        "dtype": torch.Tensor([3]),
                    },
                )
                G.add_edges(
        ...
        if sec_num != 0:
            ...
            G.nodes[secid2nid].data["label"] = torch.LongTensor(
                [[0] * self.doc_max_timesteps] * sec_num
            )  # [N, doc_max]
    ...
    def __iter__(self):
        ...
        for index, (data, w2s, s2s) in enumerate(
            zip(
                self._yield_data(),
                self._yield_w2s_tfidf(),
                self._yield_s2s_tfidf(),
            )
        ):
            if index >= iter_start and index < iter_end:
                ...
                for s_l in range(len(doc_len)):
                    ...
                    if now_l > self.doc_max_timesteps:
                        doc_len[s_l] = self.doc_max_timesteps - sum(
                            doc_len[:s_l]
                        )
                        break
                sec_max_timesteps = 0
...
class LoadHiExampleSet(torch.utils.data.Dataset):
    def __init__(self, data_root):
        ...
        self.gfiles = [
            f for f in os.listdir(self.data_root) if f.endswith("graph.bin")
        ]
