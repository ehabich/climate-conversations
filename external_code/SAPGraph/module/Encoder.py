...
from external_code.SAPGraph.module.PositionEmbedding import (
...
class sentEncoder(nn.Module):
    def __init__(self, hps, embed):
        ...
        self.position_embedding = nn.Embedding.from_pretrained(
            get_sinusoid_encoding_table(
                self.sent_max_len + 1, embed_size, padding_idx=0
            ),
            freeze=True,
        )
        ...
        self.convs = nn.ModuleList(
            [
                nn.Conv2d(
                    input_channels, out_channels, kernel_size=(height, width)
                )
        ...
        for conv in self.convs:
            ...
            init.xavier_normal_(
                conv.weight.data, gain=np.sqrt(init_weight_value)
            )
    ...
    def forward(self, input):
        ...
        enc_pos_embed_input = self.position_embedding(
            input_pos.long()
        )  # [s_nodes, D]
        enc_conv_input = enc_embed_input + enc_pos_embed_input
        enc_conv_input = enc_conv_input.unsqueeze(1)  # [s_nodes, 1, L, D]
        enc_conv_output = [
            F.relu(conv(enc_conv_input)).squeeze(3) for conv in self.convs
        ]  # kernel_sizes * [s_nodes, Co=50, W]
        enc_maxpool_output = [
            F.max_pool1d(x, x.size(2)).squeeze(2) for x in enc_conv_output
        ]  # kernel_sizes * [s_nodes, Co=50]
