...
a specified word embedding model, and then exports the expanded dictionary to a JSON file. 
The expansion is based on a similarity threshold.
...
def load_and_expand_moral_foundations_dictionary(dic_file_path, embedding_model='glove-twitter-25', similarity_threshold=0.85, num_words_to_expand=100):
    """
    Loads and expands a moral foundations dictionary using a word embedding model.
    """
        ...
        for line in file:
            if line.strip() and not line.startswith('%'):
                ...
                if line_counter < 12:
                    ...
                    cats = [moral_foundations_dict.get(cat, cat) for cat in moral_found_code]  # Ensure to get the category or keep the original
    ...
    for word, categories in word_to_moral_foundation.items():
        if word in model.key_to_index:
            similar_words = model.most_similar(positive=[word], topn=num_words_to_expand)
...
def main():
    ...
    expanded_dictionary = load_and_expand_moral_foundations_dictionary(dic_file_path, embedding_model, similarity_threshold, num_words_to_expand)
    
    with open('expanded_moral_foundations_dictionary.json', 'w', encoding='utf-8') as json_file:
        json.dump(expanded_dictionary, json_file, ensure_ascii=False, indent=4)
    
    print("Expanded dictionary loaded with", len(expanded_dictionary), "entries and dumped to JSON.")
